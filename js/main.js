...
'</tr> ' ;
        ...
        this.display_timeline = function () {
            ...
            for (var pvname in this.pvtypeinfo) {
                ...
                // skip lots which don't have any data
                if (!this.pvdata[pvname].length || ((this.pvdata[pvname].length === 1) && !this.pvdata[pvname][0].data.length)) {
                    this._selectedPVs[pvname].children('td.notes').html('<span style="color:maroon;">no data received</span>') ;
                    continue ;
                }
